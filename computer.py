...
class Computer:
    ...
    def radial_launch(self, t, X):
        ...
        a1 = self.rocket.T - (self.environment.G * self.environment.M_earth * self.rocket.M)/(r**2)

        a_x = a1*math.cos(theta)*math.cos(phi) - rho*v_rel*self.rocket.C_A*v_xrel/2
        a_y = a1*math.cos(theta)*math.sin(phi) - rho*v_rel*self.rocket.C_A*v_yrel/2
        a_z = a1*math.sin(phi) - rho*v_rel*self.rocket.C_A*v_zrel/2

        return [np.array([v_x, v_y, v_z]), np.array([a_x, a_y, a_z])]
